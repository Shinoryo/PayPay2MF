...
_MSG_MAPPING_KEYWORD_TYPE = "mapping_rules[{i}]: keyword には文字列を指定してください。"
_MSG_MAPPING_CATEGORY_TYPE = (
    "mapping_rules[{i}]: category には文字列を指定してください。"
)
...
_MSG_MAPPING_REGEX_INVALID = (
    "mapping_rules[{i}]: regex が不正です: {pattern!r} ({detail})"
)
...
def _validate_mapping_rules(rules: list) -> None:
    ...
    for i, rule in enumerate(rules):
        ...
        if mode not in AppConstants.VALID_MATCH_MODES:
            ...
            _validate_mapping_rule_regex(keyword, mode, i, errors)
...
def _validate_mapping_rule_text_fields(
    ...
    keyword = rule.get(_KEY_RULE_KEYWORD, AppConstants.EMPTY_STRING)
    category = rule.get(_KEY_RULE_CATEGORY, AppConstants.EMPTY_STRING)

    if not isinstance(keyword, str):
        errors.append(_MSG_MAPPING_KEYWORD_TYPE.format(i=index))
    elif not keyword.strip():
        errors.append(_MSG_MAPPING_KEYWORD_EMPTY.format(i=index))

    if not isinstance(category, str):
        errors.append(_MSG_MAPPING_CATEGORY_TYPE.format(i=index))
    elif not category.strip():
        errors.append(_MSG_MAPPING_CATEGORY_EMPTY.format(i=index))

    return keyword, category


def _validate_mapping_rule_regex(
    keyword: object,
    mode: object,
    index: int,
    errors: list[str],
) -> None:
    if mode != AppConstants.MATCH_MODE_REGEX or not isinstance(keyword, str):
        return
    ...
    try:
        ...
        errors.append(
            _MSG_MAPPING_REGEX_INVALID.format(
                i=index,
                pattern=keyword,
                detail=str(exc),
            )
