...
- `mapping_rules` は `priority` 降順で評価し、
  同一 `keyword`・同一 `match_mode`・同一 `priority` の競合時は
  `direction` 指定（`income` / `expense`）を `any` より優先する
- `direction` は CSV の入出金判定結果（`Transaction.direction`）と照合する
- `match_mode` は `contains`（部分一致）/ `starts_with`（前方一致）/ `regex`（正規表現）を使い分ける
- `keyword` は設定値をそのまま保持し、前後空白を含む場合は空白込みでマッチ評価する
...
- `row_index` はヘッダを除いたデータ行の 1 始まり連番で、
  登録失敗 CSV での元行特定に使用する
...
- Money Forward サインインページ（`https://moneyforward.com/sign_in`）を開く
- ユーザーが手動ログインして Enter を押した後、
  家計簿タブを経由して `/cf`（`https://moneyforward.com/cf`）へ遷移する
...
- submit 後は手入力モーダル内の `入力を保存しました。` 成功文言を成功シグナルとして扱う
- フォーム内エラー要素が表示された場合は失敗として扱う
- 15 秒以内に成功文言・エラー要素のどちらも検知できない場合は timeout エラーとする
- 成功・失敗・timeout のいずれでも、状態を初期化するため `/cf` へ再遷移して次の取引へ進む
...
- `error_*.csv`
- 列は `row_index`、`date`、`amount`、`direction`、`merchant`、
  `category`、`error_message`
- `row_index` は元 CSV のデータ行連番（ヘッダ除く、1 始まり）
