...
- `gcloud_credentials_path` が必須で、`duplicate_detection.tolerance_seconds` は
  本体と同じ検証ルールに従う
...
- [src/paypay2mf/csv_parser.py](../src/paypay2mf/csv_parser.py)
  文字コード自動判定、CSV 読込、複合支払集約、`Transaction` 変換、
...
- 同一 `transaction_id` の複数行を集約し、出金・入金金額を合算する
- `transaction_id` が空の行は独立行として扱い、仮 ID と実 ID の衝突を避ける
- 集約後の `row_index` と金額以外のメタデータは最初の行を保持する
- 後続行の `取引日`、`取引方法`、`利用者` などは上書きしない
- 3 行以上に分かれた複合支払でも同じルールで 1 件へ集約する
- したがって、複合支払の解析失敗が発生した場合の行番号は
  そのグループの先頭行を指す
...
- `transaction_id` がある取引は ID で記録する
- 欠損時は `datetime`、`amount`、`merchant` の組を記録する
...
- `transaction_id` がある場合はそれを document id として扱う
- 欠損時は fallback document id と payload を使う
- fallback 検索では `amount`、`merchant`、`date_bucket` で候補を絞り、
  最後に `datetime` と `tolerance_seconds` で許容判定する

### 9.3 date_bucket

- `datetime` の秒とマイクロ秒を 0 に丸めた分単位の検索補助キーで、
  形式は `YYYYMMDDHHMM` とする
- Firestore fallback 検索では、許容幅に含まれる `date_bucket` 候補を列挙し、
  `amount`、`merchant`、`date_bucket` で候補を絞った後に
