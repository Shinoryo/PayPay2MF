...
def test_parse_normal_payment() -> None:
    ...
    txs = parse_csv(FIXTURE_DIR / "test_input.csv", _make_config(FIXTURE_DIR / "test_input.csv"))
...
def test_parse_compound_payment(tmp_path: Path) -> None:
    ...
    txs = parse_csv(csv_file, _make_config(csv_file))
    assert len(txs) == 1
...
def test_encoding_sjis() -> None:
    ...
    txs = parse_csv(sjis_path, _make_config(sjis_path))
    assert len(txs) > 0
    assert any(t.merchant == "モスのネット注文" for t in txs)
